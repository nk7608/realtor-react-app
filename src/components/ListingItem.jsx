  ...
  return (
    <li className="relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]">
      <Link className="contents" to={`/category/${listing.type}/${id}`}>
        <img className="h-[170px] w-full object-cover hover:scale-105 transition-scale duration-200 ease-in" loading="lazy" src={listing.imgUrls[0]} alt="user-listing" />
...
{listing.timeStamp?.toDate()}
    ...
    {onEdit && (<MdEdit className="absolute bottom-2 right-7 h-[4px] cursor-pointer" onClick={()=>onEdit(listing.id)} />)}
    </li>
  )
}
